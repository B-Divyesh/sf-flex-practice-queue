...
function landing(): string {
  return shell(`
    <section class="hero blueprint-grid">
      ...
      <figure class="hero-art"><img src="/art/blueprint-practice.webp" width="1536" height="1024" alt="Prompt cards arranged around a practice timer on blueprint paper." fetchpriority="high" decoding="async"><figcaption>Choose a few existing flashcards for extra practice.</figcaption></figure>
    </section>
    <section class="workbench" aria-labelledby="workspace-heading"><div class="section-heading"><p class="drawing-label">Your practice queue</p><h2 id="workspace-heading">Build a round from your prompts</h2><p>Import a CSV or add one prompt. Your imported file is only read.</p></div><div id="practice-app" class="practice-app" aria-live="polite"><p class="loading-state">Opening your local queue…</p></div></section>
    <section id="how" class="how-section blueprint-grid" aria-labelledby="how-heading"><h2 id="how-heading">How optional practice works</h2><ol><li><b>Import prompts.</b><span>Use a CSV from your flashcard schedule. Nothing writes back.</span></li><li><b>Tag prompts.</b><span>Tag warm-up, weak, or today. Pick any mix.</span></li><li><b>Run the round.</b><span>Reveal answers at your pace. Stop when your window closes.</span></li></ol></section>
    <section class="boundary-section" aria-labelledby="boundary-heading"><div><h2 id="boundary-heading">Leaves your flashcard schedule alone</h2></div><div><p>This queue does not calculate due dates or rate cards in your flashcard schedule. It stores prompts and round notes in this browser.</p><p>Export your prompts whenever you want. Delete them from this device when needed.</p></div></section>
    ${pricingSection()}
  `);
}

function pricingSection(): string {
  return `<section class="price-section" aria-labelledby="price-heading"><div><p class="drawing-label">Paid round plans</p><h2 id="price-heading">Save round plans for $9 once</h2><p>Import, tag, practice, and export stay free. A license adds named round plans for repeated routines.</p><a class="button primary" href="${checkoutUrl}">Buy a $9 license</a><p class="fine-print">Checkout opens on Sociobot after you choose to buy.</p></div><form id="license-form" class="license-form"><label for="license-token">Have a license? Paste it</label><div><input id="license-token" name="license" autocomplete="off" required><button class="button secondary" type="submit">Verify license</button></div><p id="license-status" class="form-status" role="status" aria-label="License verification status" aria-live="polite" aria-atomic="true">${hasLicense() ? 'License active.' : 'No license is saved.'}</p></form></section>`;
}
  ...
  private bindBuilder() {
    this.root.querySelector<HTMLInputElement>('#csv-file')?.addEventListener('change', async event => {
      ...
      if (file.name.toLowerCase().endsWith('.apkg')) {
        this.status('This app cannot read .apkg packages. Export a front, back, tags CSV from Anki first.', true);
